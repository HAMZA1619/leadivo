...
export async function GET(request: Request) {
  try {
    ...
    if (!storeId || !wilayaId || !/^\d+$/.test(wilayaId)) return NextResponse.json({ error: "Missing params" }, { status: 400 })
    ...
    const res = await fetch(`https://api.yalidine.app/v1/communes/?wilaya_id=${encodeURIComponent(wilayaId)}`, {
