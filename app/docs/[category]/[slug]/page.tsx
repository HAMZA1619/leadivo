...
export default function ArticlePage({
  ...
  // Get next/prev articles for navigation
  const allArticles = getCategoryArticles(catSlug)
  ...
  return (
    <div className="space-y-10">
      ...
      <div className="flex flex-wrap items-center gap-2 text-sm text-muted-foreground">
        ...
        <Link href={`/docs/${catSlug}`} className="transition-colors hover:text-foreground">
          {category.title[lang]}
        </Link>
        <ChevronRight className="h-3 w-3 rtl:-rotate-180" />
      ...
      {(prevArticle || nextArticle) && (
        <FadeIn>
          <div className="grid gap-3 border-t pt-8 sm:grid-cols-2">
            {prevArticle ? (
              <Link
                href={`/docs/${catSlug}/${prevArticle.slug}`}
            ...
            {nextArticle && (
              <Link
                href={`/docs/${catSlug}/${nextArticle.slug}`}
