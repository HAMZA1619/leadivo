...
export function DocsCategory({ categorySlug, linkPrefix = "" }: { categorySlug: string; linkPrefix?: string }) {
  ...
  return (
    <div className="space-y-10">
      <Link href={`${linkPrefix}/docs`} className="inline-flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground">
      ...
      <div className="space-y-3">
        {articles.map((article, i) => (
          <FadeIn key={article.slug} delay={i * 40}>
            <Link
              href={`${linkPrefix}/docs/${categorySlug}/${article.slug}`}
