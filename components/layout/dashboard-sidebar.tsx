...
const navItems: NavItem[] = [
  ...
  { href: "/dashboard/store/theme", labelKey: "nav.design", icon: Paintbrush },
  {
    ...
    icon: Package,
    children: [
      { href: "/dashboard/products", labelKey: "nav.products" },
      { href: "/dashboard/collections", labelKey: "nav.collections" },
      { href: "/dashboard/reviews", labelKey: "nav.reviews" },
    ],
  },
  {
...
for (const item of navItems) {
  if (item.children) {
    groupChildPaths[item.href] = item.children.map((c) => c.href)
  }
}
...
function SidebarContent({ pathname, onNavigate }: {
  ...
  const isGroupActive = (href: string) => groupChildPaths[href]?.some((p) => isActive(pathname, p)) ?? false
  const [openGroups, setOpenGroups] = useState<Record<string, boolean>>({})

  useEffect(() => {
    setOpenGroups((prev) => {
      const next: Record<string, boolean> = { ...prev }
      for (const href of Object.keys(groupChildPaths)) {
        if (isGroupActive(href)) {
          next[href] = true
        } else {
          next[href] = false
        }
